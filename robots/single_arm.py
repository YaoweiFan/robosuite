...
class SingleArm(Robot):
    ...
    def get_observations(self, di: OrderedDict):
        ...
        if self.has_gripper:
            ...
            di[pf + "ft"] = np.array(np.concatenate((self.ee_force, self.ee_torque)))
            di[pf + "eef_ft"] = self.controller.eef_ft
            robot_states.extend([di[pf + "gripper_qpos"], di[pf + "gripper_qvel"], di[pf + "ft"]])
